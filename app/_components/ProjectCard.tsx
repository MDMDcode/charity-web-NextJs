...
interface Project {
  ...
  // التعديل هنا لضمان وجود القيم المطلوبة للإحصائيات
  target: { 
    has_target: boolean; 
    goal_amount: string; 
    ...
    <div className="relative flex-shrink-0 w-full sm:w-[350px]" dir="rtl">
      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-md border border-gray-100 flex flex-col h-full hover:shadow-lg transition-shadow">
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          {/* شريط التقدم (Progress Bar) - يظهر فقط إذا كان للمشروع هدف (Target) */}
          {project.target.has_target && (
            <div className="mb-6">
              <div className="flex justify-between items-end mb-2">
                <span className="text-xs text-gray-400">تم جمع <span className="text-[#009689] font-bold">{collected} ر.س</span></span>
                <span className="text-xs font-bold text-[#009689]">{currentPercentage}%</span>
              </div>
              <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                <div 
                  className="h-full bg-[#009689] transition-all duration-1000 ease-out"
                  style={{ width: `${currentPercentage}%` }}
                ></div>
              </div>
              <div className="mt-2 text-left">
                <span className="text-[10px] text-gray-400">المبلغ المستهدف: {goal} ر.س</span>
              </div>
            </div>
          )}

          <div className="mb-6">
            <label className="block text-xs text-gray-400 mb-2 font-medium italic">أدخل مبلغ المساهمة</label>
            <div className="relative">
              <input
                ...
                placeholder="0.00"
              />
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-bold">ر.س</span>
            </div>
          </div>
